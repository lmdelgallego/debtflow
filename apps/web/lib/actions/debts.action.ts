...
import type { CreateDebtInput, Debt, UpdateDebtInput } from '@debtflow/domain';
import { createClient } from '@/lib/supabase/server';
export type { CreateDebtInput, Debt, UpdateDebtInput } from '@debtflow/domain';
...
export async function payDebt(
  ...
  try {
    ...
    const { data: rawDebt, error: fetchError } = await supabase
      .from('debts')
      .select('*')
      .eq('id', debtId)
      .eq('user_id', user.id)
    ...
    const debt = rawDebt as Debt | null;
